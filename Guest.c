...
typedef struct {
    char id[MAX_LEN];
    int remain_time;   // 분 단위 남은 시간
    time_t last_time;  // 마지막 기록 시각 (epoch time)
} GuestInfo;

// 파일에서 게스트 정보 읽기 (존재하면 1 반환, 없으면 0)
int readGuest(const char *id, GuestInfo *guest) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        int remain;
        long long last;  // time_t는 보통 long long 혹은 long
...
void chargeTime(GuestInfo *guest) {
    ...
    printf("돈을 넣어주세요 (충전 금액 이상): ");
    scanf("%d", &money);

    if (money < price) {
        printf("돈이 부족합니다. 충전 취소.\n");
        return;
    }
...
void showRemainingTime(GuestInfo *guest) {
    ...
    if (guest->remain_time <= 0) {
        printf("비회원 [%s]의 남은 시간이 없습니다.\n", guest->id);
    } else {
        printf("비회원 [%s]의 남은 시간: %d분\n", guest->id, guest->remain_time);
    }
}
...
int main() {
    ...
    printf("비회원 ID를 입력하세요 (경고 비밀번호 없음): ");
    scanf("%s", guest_id);

    printf("\n");

    if (!readGuest(guest_id, &guest)) {
        printf("비회원 [%s] 정보를 찾을 수 없어 새로 생성합니다.\n", guest_id);
